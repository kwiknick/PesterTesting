...
<#
    ...
    $rssData += [PSCustomObject][Ordered]@{
      ...
      Author = $rss.author.Replace('&', 'and') 
      PubDate = $rss.pubDate
      Image = $rss.image.href
      Mp3Url = $rss.enclosure.url
      Mp3Length = $rss.enclosure.length 
#>
    # Hortense Good Code
    $rssData += [PSCustomObject][Ordered]@{
      # Note the & will mess up the XML, so we'll replace it where needed
      PSTypeName = 'PodcastSight.Podcast'
