...
Describe 'Get-PodcastData Tests' {
  ...
  foreach ($podcast in $rssData)
  {
    ...
    Context "Podcast Values for row $rowNum" {
      ...
      # Oops. In the video I forgot to change the test description to have the 
      # or .png on it. Doesn't affect anything as this is just to display information
      # when the test is running but thought I'd make it look nice in the download
      It 'ImageUrl should end with .jpg or .png' {
        $podcast.ImageUrl.EndsWith('.jpg') -or $podcast.ImageUrl.EndsWith('.png') |
          Should Be $true
      }
